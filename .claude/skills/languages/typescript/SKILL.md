---
name: typescript
description: TypeScript conventions, tied to no framework or domain — currently covers restricting the `tsc` CLI to non-emitting, check-only invocations (never a bare `tsc` or `npm run build` that emits .js files). Load this skill whenever the user asks to type-check, verify types, or run tsc in a TS/TSX project that uses Vite/bundler module resolution. For React-specific TypeScript conventions (named exports, Props interfaces, avoiding `any`), see [[frontend/react/typescript]].
---

# TypeScript (frontend)

## Check-only `tsc` usage
