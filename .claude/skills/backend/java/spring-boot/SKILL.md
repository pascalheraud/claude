...
Spring Boot/Java-specific implementation of the [[api]] conventions. Load [[api]] first — this skill only adds what's specific to Spring Boot; it doesn't repeat the route-guard/status-code/response-shape/DI rules.

## Controllers vs Services

Same split as [[api]]'s "route handler vs service layer": a controller **calls the repository directly** if and only if the endpoint makes **a single repository call**; anything more goes into a `@Service` bean.
...
- **All controller inputs are validated:**
  ...
  - When the request body is a **dedicated payload** (single-purpose) with no nested group-validated entity, use a **bean class** (not a record) with `@Data @NoArgsConstructor` (Lombok), annotate fields with plain (group-less) Jakarta constraints (`@NotBlank`, `@NotNull`, etc.) and use `@Valid` on the controller parameter.
  - **A dedicated payload nesting a reusable group-validated entity** (e.g. a payload wrapping `SearchFilter`, which validates differently for `ByAddressValidation` vs `ByCityValidation`): reuse that same group end-to-end rather than introducing a separate mechanism. Annotate the payload's own fields with `groups = TargetEntity.SomeGroup.class` too (not group-less — `@Validated(group)` on the controller parameter validates *only* that group, so a group-less constraint, which belongs to `Default`, is silently skipped otherwise) and use `@Validated(TargetEntity.SomeGroup.class)` (not plain `@Valid`) on the controller parameter, alongside `@Valid` on the nested field to trigger the cascade. This applies even though the payload itself only has one validation scenario — the group is dictated by the nested entity's own constraint groups, not by whether the payload needs multiple scenarios. Verify empirically with a test that omits one of the payload's own fields (not just a nested field) — it's easy to end up with a passing "missing nested field" test while a missing payload-level field silently returns 200.
- Route guards (404/403/409, ownership, state-gating) and the "no intentional 500" rule follow [[api]] as-is — implement them with `ResponseStatusException`/`@ResponseStatus`/a `ResponseEntity` status, never an uncaught plain exception standing in for a real status code.
...
Same principle as [[api]]'s "response shape" rule, in Spring Boot terms:
...
Spring Boot implementation of [[api]]'s "time as an injectable dependency" rule: never call `new Date()` or `LocalDateTime.now()` directly in business code.
...
Spring Boot implementation of [[api]]'s "explicit wiring" rule: always use **constructor injection** — never `@Autowired` on fields.
...
Spring Boot implementation of [[api]]'s "API tests" focus:
