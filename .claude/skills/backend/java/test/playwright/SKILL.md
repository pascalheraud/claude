---
name: playwright
description: Java End-to-End testing conventions using Playwright for Java — Testcontainers to stand up the DB and the app (npm-built frontend), one scenario class per scenario. Builds on the generic [[test/e2e]] and [[test/e2e/playwright]] skills.
---

# Java E2E tests with Playwright

Java-specific implementation of the [[test/e2e]] and [[test/e2e/playwright]]
conventions. Load both first — this skill only adds what's specific to a
JVM/Testcontainers/JUnit stack; it doesn't repeat the scenario/PageObject/
snapshot rules or the generic Browser/Context/Page lifecycle.

[Playwright for Java](https://playwright.dev/java/) is the library used to
drive the browser — the Java binding of [[test/e2e/playwright]]'s
...
- **A DB container** (e.g. `PostgreSQLContainer`), migrated to the real schema — the same container/`DataSource` the project's test-data loader uses for E2E tests. Always a Testcontainer, regardless of how the app itself is run.
...
[[test/e2e]]'s PageObject conventions (private constructor, `static open(...)` factory, destination-typed returns) and [[test/e2e/playwright]]'s "holds a `Page`, not a `Browser`/`BrowserContext`" rule apply as-is in Java — see [[test/e2e/playwright]] for the shared shape. The only Java-specific detail is the return type of chained methods, shown in the Given/When/Then example above (`LoginPage.open(...)`, `.fillPassword(...)`).
